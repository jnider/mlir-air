...
main(int argc, char *argv[])
{
  ...
  #define DMA_COUNT 16
  #define BUFFER_COUNT (2*DMA_COUNT)
  ...
  if(hsa_ret != HSA_STATUS_SUCCESS) {
    printf("[ERROR] air_explore_world failed\n");
    return -1;
  }
  ...
  hsa_ret = air_ernic_mem_alloc("host0_src", 4096, &src_tensor, false); // These need to be larger than 256B as that is the smallest send size.
  hsa_ret = air_ernic_mem_alloc("host0_dst", 4096, &dst_tensor, false);
  ...
  //
  // Sending 2nd half of elements
  //
  air_send(NULL, &src_tensor, DMA_COUNT * sizeof(uint32_t), DMA_COUNT * sizeof(uint32_t), 1, queues[0], 0);
  ...
  //
  printf("[INFO] Performing RECV on dst_tensor\n");
  air_recv(NULL, &dst_tensor, DMA_COUNT * sizeof(uint32_t), DMA_COUNT * sizeof(uint32_t), 1, queues[0], 0);
