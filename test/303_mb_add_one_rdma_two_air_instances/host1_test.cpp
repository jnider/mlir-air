...
main(int argc, char *argv[])
{
  ...
  #define DMA_COUNT 16
  #define BUFFER_COUNT (DMA_COUNT*2)
  ...
  // If the program crashes can uncomment these and run the program to bring the ERNIC 
  ...
  hsa_ret = air_ernic_mem_alloc("host1_barrier_tensor", RQE_SIZE, &barrier_dummy_tensor, false);
  ...
  uint32_t *dst_tensor_local_buff = (uint32_t *)(tensor_to_qp_map[dst_tensor.alloc]->local_buff->buff);

  for (int i=0; i<BUFFER_COUNT; i++) {
    dst_tensor_local_buff[i] = 0xdeface;
    src_tensor.alloc[i] = i;
  }

  // After we initialize the input want to sync up with the other devices
  air_barrier(&barrier_dummy_tensor, queues[0], 1);
  ...
  air_packet_nd_memcpy(pkt1, 0, col, 1, 0, 4, 2, tensor_to_qp_map[src_tensor.alloc]->local_buff->pa + DMA_COUNT*sizeof(float), DMA_COUNT*sizeof(float), 1, 0, 1, 0, 1, 0);
  ...
  air_packet_nd_memcpy(pkt2, 0, col, 0, 0, 4, 2, tensor_to_qp_map[dst_tensor.alloc]->local_buff->pa + DMA_COUNT*sizeof(float), DMA_COUNT*sizeof(float), 1, 0, 1, 0, 1, 0);
  ...
  printf("Generating WRITE POST_WQE HSA packet\n");
  wr_idx = queue_add_write_index(queues[0], 1);
  packet_id = wr_idx % queues[0]->size;
  dispatch_packet_t *pkt = (dispatch_packet_t*)(queues[0]->base_address_vaddr) + packet_id;
  air_packet_post_rdma_wqe(pkt,                                                 // HSA Packet
                          tensor_to_qp_map[dst_tensor.alloc]->vaddr + DMA_COUNT*sizeof(float),            // Remote VADDR
                          tensor_to_qp_map[dst_tensor.alloc]->local_buff->pa + DMA_COUNT*sizeof(float),   // Local PADDR
                          DMA_COUNT*sizeof(float),                              // Length
                          (uint8_t)OP_WRITE,                                    // op
                          (uint8_t)tensor_to_qp_map[dst_tensor.alloc]->rkey,    // Key
                          (uint8_t)tensor_to_qp_map[dst_tensor.alloc]->qp,      // QPID
                          (uint8_t)1);                                          // ERNIC select
  air_queue_dispatch_and_wait(queues[0], wr_idx, pkt);

  // After we write to the output want to sync up with the other devices
  air_barrier(&barrier_dummy_tensor, queues[0], 1);
  ...
  // For this test only go thorugh half of the buffer
  // because the entire destination is on the other
  for (int i=DMA_COUNT; i<BUFFER_COUNT; i++) {
    //uint32_t s = src_tensor.d[i];
    volatile uint32_t s = src_tensor.alloc[i];
    uint32_t d = dst_tensor_local_buff[i];
