...
#define DMA_COUNT 32
  ...
  uint32_t *dram_ptr_1 =
      (uint32_t *)air_malloc(DMA_COUNT * sizeof(uint32_t));
  uint32_t *dram_ptr_2 =
      (uint32_t *)air_malloc(DMA_COUNT * sizeof(uint32_t));
  uint32_t *dram_ptr_3 =
      (uint32_t *)air_malloc(DMA_COUNT * sizeof(uint32_t));
  ...
  for (int i=0;i<DMA_COUNT;i++) {
    ...
    dram_ptr_4[i] = 0x10051ade;
    dram_ptr_5[i] = 0x2001ba11;
    dram_ptr_6[i] = 0x20051ade;
  }
  ...
  air_packet_nd_memcpy(
      ...
      DMA_COUNT * sizeof(float), 1, 0, 1, 0, 1, 0);
  hsa_amd_signal_create_on_agent(1, 0, nullptr, &agents[0], 0, &pkt_h.completion_signal);
