...
#define DMA_COUNT 16
  ...
  air_queue_dispatch_and_wait(&agents[0], queues[0], packet_id, wr_idx, &write_pkt);
  ...
  air_packet_nd_memcpy(&read_pkt, 0, col, 0, 0, 4, 2, reinterpret_cast<uint64_t>(dst),
                       DMA_COUNT * sizeof(float), 1, 0, 1, 0, 1, 0);
  air_queue_dispatch_and_wait(&agents[0], queues[0], packet_id, wr_idx, &read_pkt);
