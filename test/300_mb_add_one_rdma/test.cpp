...
main(int argc, char *argv[])
{
  ...
  hsa_ret = air_ernic_mem_alloc("src", DMA_COUNT * sizeof(uint32_t), &src_tensor, false);
  if(hsa_ret != HSA_STATUS_SUCCESS) {
    printf("[ERROR] Failed to allocate src tensor\n");
    return -1;
  }
  hsa_ret = air_ernic_mem_alloc("dst", DMA_COUNT * sizeof(uint32_t), &dst_tensor, false);
  if(hsa_ret != HSA_STATUS_SUCCESS) {
  ...
  for (int i=0; i<DMA_COUNT; i++) {
    src_tensor_local_buff[i] = 0xdeface;
