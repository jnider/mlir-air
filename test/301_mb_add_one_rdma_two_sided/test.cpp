...
main(int argc, char *argv[])
{
  ...
  hsa_ret = air_ernic_mem_alloc("src", 4096, &src_tensor, false);
  if(hsa_ret != HSA_STATUS_SUCCESS) {
    printf("[ERROR] Failed to allocate src tensor\n");
    return -1;
  }
  hsa_ret = air_ernic_mem_alloc("dst", 4096, &dst_tensor, false);
  if(hsa_ret != HSA_STATUS_SUCCESS) {
    printf("[ERROR] Failed to allocate dst tensor\n");
    return -1;
  }
  ...
  air_recv(NULL, &src_tensor, DMA_COUNT * sizeof(uint32_t), 0, 1, queues[0], 0);
  ...
  air_send(NULL, &dst_tensor, DMA_COUNT * sizeof(uint32_t), 0, 1, queues[0], 0);
